concurrent() (
	...
	__crt__args__handle_task_flag() {
		set -- "${remaining_args[@]}"
		shift
		(($#)) || __crt__error "expected task name after '-'"
		__crt__names+=("${1}")
		shift
		(($#)) || __crt__error "expected command after task name"
		local args=()
	...
	__crt__args__handle_require_flag() {
		...
		while (($#)) && __crt__args__is_require_flag "${1}"; do
			shift
			(($#)) || __crt__error "expected task name after '--require'"
		...
		if __crt__args__is_before_all_flag "${1}"; then
			...
			while (($#)) && __crt__args__is_before_flag "${1}"; do
				shift
				(($#)) || __crt__error "expected task name after '--before'"
